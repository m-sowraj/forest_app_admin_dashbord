...
function SignIn() {
  ...
  const handleLogin = async () => {
    try {
        let userType = 'forestOfficial'
      const response = await axios.post('https://elephant-tracker-api.onrender.com/api/userauth/login', { phone_num:phoneNumber, password });
      ...
      await  sessionStorage.setItem('key', userType === 'resident' ? "1" : "2");
      ...
      <div style={{ backgroundColor: '#F5DBC4', borderRadius: '24px' }} className='flex'>
        <div className='bg-white flex items-center justify-center' style={{ borderRadius: '24px 0 0 24px' }}>
          <div className='p-8 flex flex-col gap-6 text-sm'>
            ...
            <button onClick={handleLogin} className={Styles.btn} style={{ marginTop: '1em' }}>Login</button>
